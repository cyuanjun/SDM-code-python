...
- ~~**[US-30.jpg](../diagrams/sprint-3_diagrams/US-30.jpg) boundary class collision:** class + sequence diagrams name the boundary `ViewMyFundraisingActivitiesPage`~~ **Resolved 2026-05-16 / further 2026-05-17.** Re-exported diagram 2026-05-17 names the boundary `ViewMyCompletedFundraisingActivitiesPage` (shared with US-31), drops the FRA short form on the entity/controller method (now `searchMyCompletedFundraisingActivity`), and renames the boundary display method `displayMatchingMyCompletedFundraisingActivity`. Code aligned: entity / controller method renamed from `search_my_completed_fra`; boundary class renamed from `SearchMyCompletedActivityPage` → `ViewMyCompletedFundraisingActivitiesPage` (single file shared with US-31, mirrors US-14/17 pattern). Deferred code-vs-diagram boundary-class divergence is now resolved.
...
- ~~**[US-31.jpg](../diagrams/sprint-3_diagrams/US-31.jpg) per-id detail vs list semantics**~~ **Resolved 2026-05-17.** Re-exported diagram reframes US-31 from "view one completed activity by FRAId" to "view the list of my completed activities" — method renamed `viewMyCompletedActivity(ownerAccountId, FRAId): FundraisingActivity` → `viewMyCompletedFundraisingActivities(ownerAccountId): List<FundraisingActivity>` (entity + controller). Code aligned: old method + old controller + old per-US page deleted; new method + new controller + merged boundary shared with US-30. The detail-panel call in `ManageMyFundraisingActivityPage` now reuses US-13's `view_my_fundraising_activity(owner, fra_id)` since the picker source is already the completed-only list.
