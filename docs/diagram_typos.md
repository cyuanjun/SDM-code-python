...
- **[US-30.jpg](../diagrams/sprint-3_diagrams/US-30.jpg) boundary class collision:** class + sequence diagrams name the boundary `ViewMyFundraisingActivitiesPage` — same name as the US-17 boundary. Implementation uses `SearchMyCompletedActivityPage` to avoid the collision and match the user story ("search my completed activities"). Rename in the diagram or document the collision.
- **[US-30.jpg](../diagrams/sprint-3_diagrams/US-30.jpg) `searchMyCompletedFRA` parameter order:** diagram signs the method `searchMyCompletedFRA(searchCriteria: String, ownerAccountId: String)` (criteria first). Implementation reverses to `search_my_completed_fra(owner_account_id, search_criteria)` so it matches every other owner-scoped method (`search_my_fundraising_activity`, `update_my_fundraising_activity`, `suspend_my_fundraising_activity`, etc.) which all put `owner_account_id` first. Reorder on the diagram, or document the project convention.
- **[US-32.jpg](../diagrams/sprint-3_diagrams/US-32.jpg) controller class name + sequence/class diagram inconsistency:** class diagram labels the controller box `SearchMyDonationHistoryController` (with `My`), but the diagram's own class-level entity method is `searchDonationHistory` (without `My`) while the sequence's call is `searchMyDonationHistory` (with `My`). Implementation drops `My` throughout: `SearchDonationHistoryController` + `Donation.search_donation_history`. Reconcile the diagram internally and pick one convention.
...
- **[US-41.jpg](../diagrams/sprint-4_diagrams/US-41.jpg) / [US-42.jpg](../diagrams/sprint-4_diagrams/US-42.jpg) / [US-43.jpg](../diagrams/sprint-4_diagrams/US-43.jpg) `Report.generatedAt: Date`.** All three diagrams type `Report.generatedAt` as `Date`. Implementation stores a full `datetime.datetime` (set via `datetime.now()`) in [entity/report.py](../entity/report.py) so the timestamp captures the exact generation moment. Either retype to `DateTime` on the diagrams, or truncate to a `date` in code.
