...
CSIT314 group project — online fundraising platform (Python + Streamlit + SQLite, B-C-E architecture). All **43 user stories** across **4 sprints** implemented; **392 tests passing**; **10 sidebar entries** routed by role. Per-US detail starts at [Sprint 1](#sprint-1) below — this Overview is the bird's-eye view.
...
- **Coverage:** all 43 user stories across 4 sprints implemented. **392 tests passing** locally and in CI.
...
- **Run:** `pytest` (full suite, 392 tests) or `pytest -v` (matches CI). Single test: `pytest tests/test_user_account.py::test_login_succeeds`. By keyword: `pytest -k favourite`.
