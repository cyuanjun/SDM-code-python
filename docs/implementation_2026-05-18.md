...
- **Lecturer decisions (4)** — donation seed (2026-05-15), `UNIQUE(email)` on UserAccount (2026-05-15; companion `UNIQUE(phone_num)` added 2026-05-18 as a parallel real-world business rule), no `displayError` on Sprint 1 boundaries (2026-05-16), login failure return type implicit on US-11/18/26/39 (2026-05-16).
...
**Tests**
- [tests/test_user_account.py](../tests/test_user_account.py) — happy + missing-id `False` + email-conflict `False` + phone-conflict `False` (both UNIQUE constraints)
...
**Notes / assumptions / deferred:** Boundary validation enforces `@` in email + non-empty `email/password/name/phone_num`. The entity catches `sqlite3.IntegrityError` from the `UNIQUE(email)` (lecturer 2026-05-15) AND `UNIQUE(phone_num)` (2026-05-18) constraints and returns `False`. The consolidated `ManageUserAccountPage` re-queries the DB after a False return to disambiguate which column collided and shows a targeted "that email / phone number is already in use by another account" message (via `_which_unique_collided(exclude_account_id=…)`). The picker reuses Exception A `view_all_user_accounts`. Profile dropdown is populated by `ViewProfilesController.view_all_profiles` (Exception A from US-6).
...
**Notes / assumptions / deferred:** Boundary validation requires non-empty `title/description`, a selected `fra_cat_id` (selectbox over active categories, allowing the existing one even if now suspended so the activity can keep its category), a positive `Decimal` target, `start_date <= end_date`, and `end_date >= today` (added 2026-05-18 — an already-running activity may have a past `start_date`, but you can't move its end date into the past). **Field-unpacked signature (2026-05-18):** the entity + controller method now takes the 6 editable fields directly (`title, description, target_amount, fra_cat_id, start_date, end_date`) instead of a `FundraisingActivity` DTO — matches the diagram contract exactly. `completed` and `suspended` are not settable through update; `completed` is **derive+store** — `(end_date < today)` is computed at INSERT/UPDATE time and written to the schema column, and `FundraisingActivity.refresh_completed_flags()` runs on app startup to flip rows whose `end_date` passed since the last write. `suspended` is owned by US-16 / Exception A unsuspend. Entity refuses cross-owner writes via `WHERE fra_id = ? AND owner_account_id = ?` → `rowcount == 0` → `False`. Diagram originally omitted `ownerAccountId` from the class signature and used `FundraiserActivity` type names; re-exported 2026-05-16 fixed the 3-param signature + the type. Further refined 2026-05-17: method renamed `updateFundraisingActivity` → `updateMyFundraisingActivity` for consistency with the per-US class names and to mark this as an owner-scoped operation.
...
**Notes / assumptions / deferred:** Re-exported diagram 2026-05-18 places US-30 on `ViewMyFundraisingActivitiesPage` (the same boundary as US-17 and US-31). The page renders an `[All]` / `[Completed]` tab toggle plus a search box — search-while-Completed fires US-30, search-while-All fires US-17, default Completed view fires US-31. The previous dedicated `ViewMyCompletedFundraisingActivitiesPage` retired; its tests folded into `test_view_my_fundraising_activities_page.py`. **Derive+store `completed` (2026-05-18):** the entity query filters `WHERE a.completed = 1` against the stored column. `completed` is computed as `(end_date < today)` at INSERT/UPDATE time and refreshed on app startup via `FundraisingActivity.refresh_completed_flags()` — keeps the stored value current after the calendar advances. Same shape of change as US-31.
...
- [entity/fundraising_activity.py:204](../entity/fundraising_activity.py#L204) — owner-scoped + `WHERE completed = 1` filter against the stored column (`completed` is derive+store, computed at write time + refreshed on app startup); returns list
