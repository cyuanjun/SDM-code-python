...
- **Still deferred (Sprint 4+ or hardening):** US-32 / US-33 (donee donation history) — confirmed 2026-05-11 still blocked on a missing donate use case + `Donation` entity. See [docs/issues.md](issues.md). **US-39 / US-40 (PM login / logout) are implemented via the shared `LoginPage` + `LoginController` + `UserAccount.login`** — exactly as the Sprint 1 US-39 sequence diagram specifies. PMs are seeded as `user_account` rows with `profile.role = 'platform_manager'` (2026-05-12 cleanup).
...
- **Sprint 4 `generate_*_report` signatures gain a `platform_manager_id` parameter.** The US-41 / US-42 / US-43 class diagrams show `generateDailyReport(startDate: String, endDate: String): Report` and equivalents — no PM id on the wire. Implementation threads `platform_manager_id` from `st.session_state["user"].account_id` through Boundary → Controller → Entity so `Report.platformManagerId` is the actual logged-in PM rather than a fabricated value. Update the three report class + sequence diagrams to include the extra parameter before final marking. Added 2026-05-12 as part of the `platform_manager` table cleanup (see resolution in [issues.md](issues.md)).
- **`report` table added to the schema (2026-05-12).** Every `generate_*_report` call now INSERTs a row and returns the `Report` with the real `report_id` from the insert, matching the class diagram's `reportId: Integer`. Add the new `report` table to the persistent / data design diagrams before final submission. No new use case was added — past rows are inspectable via the `.info (debug)` page only. Resolution detail in [issues.md](issues.md) "Resolved".
