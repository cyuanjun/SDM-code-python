"""Smoke + validation tests for SearchFavouritePage (US-25)."""
...
def test_page_class_is_importable_and_has_render() -> None:
    assert callable(SearchFavouritePage().render)


def test_validate_criteria_accepts_non_empty_input() -> None:
    assert SearchFavouritePage.validate_criteria("hospital") is True


def test_validate_criteria_rejects_blank_input() -> None:
    assert SearchFavouritePage.validate_criteria("") is False


def test_render_does_not_raise_when_not_logged_in() -> None:
    script = """
from boundary.search_favourite_page import SearchFavouritePage
SearchFavouritePage().render()
"""
