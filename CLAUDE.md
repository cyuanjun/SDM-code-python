...
│   ├── ids.py             # format_id(prefix, rowid) / parse_id(id_str) — shared prefixed-string ID helpers
...
- **Prefixed string IDs.** Every PK column is INTEGER in SQLite but surfaced as `prefix_NNN` strings (`prof_001`, `acc_001`, `fra_001`, `cat_001`, `rep_001`, `don_001`) via `persistence/ids.py`. Entity method signatures take and return the prefixed strings; entities call `parse_id` before binding to SQL and `format_id` when constructing the dataclass from a row. Tests assert against the prefixed form.
